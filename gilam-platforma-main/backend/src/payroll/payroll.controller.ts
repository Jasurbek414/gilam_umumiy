...
export class PayrollController {
  ...
  async calculate(@Body() body: { companyId: string; year: number; month: number; globalRestDay?: string }, @Req() req: any) {
    ...
    return this.payrollService.calculate(targetCompanyId, body.year, body.month, userId, body.globalRestDay);
  }

  @Get('periods/:companyId')
  async findPeriods(@Param('companyId') companyId: string, @Req() req: any) {
    const targetCompanyId = req.user?.role === 'SUPER_ADMIN' ? companyId : req.user?.companyId;
    return this.payrollService.findPeriods(targetCompanyId);
  }
  ...
  async approve(@Param('periodId') periodId: string, @Body() body: { companyId: string }, @Req() req: any) {
    const userId = req.user?.id || req.user?.sub;
    const targetCompanyId = req.user?.role === 'SUPER_ADMIN' ? body.companyId : req.user?.companyId;
    return this.payrollService.approve(periodId, userId, targetCompanyId);
  }
  ...
  async markAsPaid(@Param('periodId') periodId: string, @Body() body: { companyId: string }, @Req() req: any) {
    const userId = req.user?.id || req.user?.sub;
    const targetCompanyId = req.user?.role === 'SUPER_ADMIN' ? body.companyId : req.user?.companyId;
    return this.payrollService.markAsPaid(periodId, userId, targetCompanyId);
  }

  @Get('employee/:employeeId/balance')
  async getEmployeeBalance(@Param('employeeId') employeeId: string, @Query('companyId') companyId: string, @Req() req: any) {
    const targetCompanyId = req.user?.role === 'SUPER_ADMIN' ? companyId : req.user?.companyId;
    return this.payrollService.getEmployeeBalance(employeeId, targetCompanyId);
  }
}
